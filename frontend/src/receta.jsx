...
function RecetaForm() {
    ...
    useEffect(() => {
        ...
        const cargarDatosCompletos = async (cId) => {
            try {
                const resConsulta = await axios.get(`http://192.168.1.71:5000/api/consultas/${cId}`, config);
                const consultaData = resConsulta.data;
                setConsulta(consultaData);
                const resPaciente = await axios.get(`http://192.168.1.71:5000/api/pacientes/${consultaData.paciente_id}`, config);
                setPaciente(resPaciente.data);
                const resDoctor = await axios.get(`http://192.168.1.71:5000/api/usuario/actual`, config);
        ...
        const cargarDatosSimples = async (pId) => {
            try {
                const resPaciente = await axios.get(`http://192.168.1.71:5000/api/pacientes/${pId}`, config);
                setPaciente(resPaciente.data);
                const resDoctor = await axios.get(`http://192.168.1.71:5000/api/usuario/actual`, config);
                setDoctor(resDoctor.data);
                setLoading(false);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await axios.post('http://192.168.1.71:5000/api/recetas', datosReceta, config);
